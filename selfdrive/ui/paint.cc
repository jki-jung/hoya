...
static void ui_draw_vision_lane_lines(UIState *s) {
  ...
  if (scene.controls_state.getEnabled()) { 
    if (steerOverride) {
      ...
      if (!scene.lateralPlan.lanelessModeStatus) {
        track_bg = nvgLinearGradient(s->vg, s->fb_w, s->fb_h, s->fb_w, s->fb_h*.4,
                    nvgRGBA(red_lvl, green_lvl, 0, 200), nvgRGBA(230, 230, 0, 50)); // nvgRGBA((int)(0.7*red_lvl), (int)(0.7*green_lvl), 0, 1));
      } else { //laneless status
        track_bg = nvgLinearGradient(s->vg, s->fb_w, s->fb_h, s->fb_w, s->fb_h*.4,
                    nvgRGBA(red_lvl, 50, green_lvl, 200), nvgRGBA(230, 230, 0, 50)); // nvgRGBA((int)(0.7*red_lvl), 50, (int)(0.7*green_lvl), 1));
      }
    }
  } else {
