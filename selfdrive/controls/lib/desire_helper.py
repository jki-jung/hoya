...
class DesireHelper:
  ...
  def update(self, CP, carstate, controlstate, lane_change_prob, md):
    ...
    if right_edge_prob > 0.35 and right_nearside_prob < 0.2 and right_close_prob > 0.2 and left_nearside_prob >= right_nearside_prob:
      road_edge_stat = 1
    elif left_edge_prob > 0.35 and left_nearside_prob < 0.2 and left_close_prob > 0.2 and right_nearside_prob >= left_nearside_prob:
      road_edge_stat = -1
    else:
      road_edge_stat = 0
